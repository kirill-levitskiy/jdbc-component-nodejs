...
/*
    fs.readdir("/app/node_modules/jdbc/node_modules/java/build", (err, files) => {
        if (err) throw err;
        console.log("build:", files);
    });

    fs.readFile("/app/node_modules/jdbc/node_modules/java/build/jvm_dll_path.json", (err, data) => {
        if (err) throw err;
        console.log("jvm_dll_path.json:", data.toString());
    });

    fs.readFile("/app/node_modules/jdbc/node_modules/java/build/Release/nodejavabridge_bindings.node", (err, data) => {
        if (err) throw err;
        console.log("nodejavabridge_bindings.node:", data.toString());
    });
*/

    //console.log("jvm_dll_path.json:", jvm_dll_path);

    //console.log("Classpath was set", jinst.getInstance());
                                            ...
                                            if (err) {
                                                ...
                                                resultset.toObjArray(function (err, results) {
                                                    if (results.length > 0) {
                                                        ...
                                                        return (results) => messages.newMessageWithBody(results);
                                                    }
