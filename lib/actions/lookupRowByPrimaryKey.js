...
function processAction(msg, cfg, snapshot = {}) {
    ...
    const conString = "jdbc:sqlserver://elasticiotest.database.windows.net:1433;database=Test2;user=elasticiotest;password=Init123$;encrypt=true;trustServerCertificate=false;hostNameInCertificate=*.database.windows.net;loginTimeout=30;";
    var mssql = new jdbc({
        url: conString,
...
*/
    ...
    request.createStatement(function(err, statement) {
        if (err) {
            callback(err);
        } else {
            statement.executeQuery(originalSql, function(err, resultset) {
    ...
    }).then((resultset) => messages.newMessageWithBody(resultset));
